...
var fade_duration : float = 1.0 # Durée du fade (apparition et disparition)
...
var has_triggered : bool = false # Pour s'assurer que ça ne se déclenche qu'une fois
...
func _on_player_on_pick_object() -> void:
	# Ce code est exécuté lorsque le joueur prend un objet
	if not has_triggered: # Vérifier si l'événement a déjà été déclenché
		visible = true
		has_triggered = true # Empêche de réafficher le texte à nouveau
		
		# Démarrer le fade-in
		is_fading_in = true
		fade_progress = 0.0 # Réinitialiser la progression du fade
